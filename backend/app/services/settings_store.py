...
DEFAULTS: dict[str, str] = {
    ...
    # كم يوم أجر يُخصم عن يوم «تحتاج مراجعة» (بدأ استراحة ولم يعد حتى نهاية الوردية).
    # 0.5 = نصف يوم، 1 = يوم كامل، 0 = بلا خصم (مراجعة يدوية فقط)
    "open_break_deduction_days": "0.5",
    # ------------------------------ الرواتب ------------------------------
    ...
    # النسب تُدخلها المنشأة من حسابها في التأمينات، فهي تختلف بالجنسية وبتاريخ
    # الاشتراك وتتغيّر بتعديلات النظام. يبقى الخصم صفراً حتى تُضبط.
    "gosi_enabled": "false",
    "gosi_base": "basic",              # basic = الأساسي، total = الأساسي + البدلات
    "gosi_employee_rate": "0",         # حصة الموظف السعودي %
    "gosi_employer_rate": "0",         # حصة المنشأة عن السعودي %
    "gosi_employee_rate_expat": "0",   # حصة الموظف غير السعودي % (الأصل: صفر)
    "gosi_employer_rate_expat": "0",   # حصة المنشأة عن غير السعودي %
    "gosi_max_base": "45000",          # الحد الأقصى للوعاء
    "gosi_prorate": "true",            # تقسيط الوعاء على أيام الاستحقاق في الشهر الجزئي
    # ------------------------------ التقارير الموقّعة ------------------------------
    ...
    # نظام أرصدة الإجازات: معطّل، والاعتماد على طلبات الموظفين وقرار الإدارة.
    # عند تفعيله تعود الأرصدة للظهور ويعود رفض الطلب عند نفاد الرصيد،
    # والسجلات محفوظة فلا يضيع شيء بالإيقاف.
    "leave_balances_enabled": "false",
